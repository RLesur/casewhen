...
During data wrangling with `dplyr`, one may use several times identical `case_when()` clauses in different stages. This can lead to a non-DRY code. This package provides a convenient mean to define and reuse `dplyr::case_when()` functions.  
...
This package allows you to use a `case_when` function with `dbplyr`. You first have to register the `SQL` translation:
